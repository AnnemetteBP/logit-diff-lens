...
def _rank_text_color(value: int, *, cmap: LinearSegmentedColormap, norm: Normalize) -> str:
    ...
    return LIGHT if luminance < 0.58 else DARK


def plot_autoregressive_patch_scope_paper_soft(
    *,
    input_path: str | Path,
    output_png: str | Path,
    ...
    reversion_cmap = LinearSegmentedColormap.from_list(
        "patchscope_reversion_soft",
        [RISKY, SPORTS],
    )
    rank_cmap = LinearSegmentedColormap.from_list(
        "patchscope_rank_soft",
        [SPORTS, RISKY, MEDICAL],
    )
    ...
    rev_im = ax_rev.imshow(reversion, cmap=reversion_cmap, vmin=0, vmax=1, aspect="auto")
    rank_norm = Normalize(vmin=1, vmax=max_rank)
    rank_im = ax_rank.imshow(base_ranks, cmap=rank_cmap, norm=rank_norm, aspect="auto")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a soft-color paper patch-scope figure.")
    ...
    plot_autoregressive_patch_scope_paper_soft(
