...
_PAPER_SOFT_BLUE = "#5B6C9E"
_PAPER_SOFT_CREAM = "#E9E2DA"
_PAPER_SOFT_RED = "#D97C6C"


def _resolve_colorscale(colorscale: Any) -> Any:
    if colorscale in {"paper_soft", "paper_soft_r"}:
        return [
            [0.0, _PAPER_SOFT_RED],
            [0.5, _PAPER_SOFT_CREAM],
            [1.0, _PAPER_SOFT_BLUE],
        ]
    if colorscale in {"paper_soft_reversed", "paper_soft_reverse"}:
        return [
            [0.0, _PAPER_SOFT_BLUE],
            [0.5, _PAPER_SOFT_CREAM],
            [1.0, _PAPER_SOFT_RED],
        ]
    return colorscale
...
def _paper_annotation_text_color(
    value: float,
    *,
    colorscale: Any,
    zmin: float,
    ...
    if mode == "minmax_mid_dark":
        if zmax <= zmin:
            norm = 0.5
        else:
            norm = max(0.0, min(1.0, (value - zmin) / (zmax - zmin)))
        return "black" if 0.22 <= norm <= 0.78 else "white"
    return _text_color_for_value(value, colorscale=colorscale, zmin=zmin, zmax=zmax)
...
def plot_logitdiff_top_layer_selected_rows_heatmap(
    ...
    *,
    ...
    title: str | None = None,
    colorscale: Any = "RdBu",
    annotation_text_mode: str = "luminance",
    annotation_font_boost: int = 0,
    ...
    annotation_font_size = (50 if top_k <= 5 else 40) + annotation_font_boost
    ...
    for row_idx, (start, end) in enumerate(chunks, start=1):
        ...
        for col_idx in range(end - start):
            ...
            color = _paper_annotation_text_color(
                float(data["z"][layer_idx, start + col_idx]),
                colorscale=colorscale,
                zmin=0.0,
                zmax=1.0,
                mode=annotation_text_mode,
            )
