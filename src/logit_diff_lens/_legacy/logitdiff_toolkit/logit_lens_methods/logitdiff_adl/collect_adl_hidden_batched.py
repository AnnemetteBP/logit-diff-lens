from typing import List, Tuple
from pathlib import Path
import gc
from tqdm import tqdm
import torch
from transformers import PreTrainedTokenizer

from ..wrapper import (
    LogitLensWrapper,
    as_tensor,
    normalize_activations
)



@torch.no_grad()
def _collect_hidden_for_adl(
    arch_wrapper:"LogitLensWrapper",
    prompts:List[str],
    custom_tokenizer:PreTrainedTokenizer=None,
    batch_index:int=0,
    add_special_tokens:bool=False,
    analyze_special_tokens:bool=False,
    device:str|None=None,
    force_include_input:bool=True,
    force_include_output:bool=True,
    save_path=None,
    norm_modes:Tuple[str, ...]=("raw", "unit_norm", "eps_norm", "model_norm"),
    dataset:str|None=None,
) -> Tuple[List[dict], dict]:
    
    device = arch_wrapper.model_device
    model = arch_wrapper.model
    tokenizer = arch_wrapper.tokenizer if not custom_tokenizer else custom_tokenizer
    model.eval()

    if not arch_wrapper.is_bnb_quantized:
        model = model.to(device)

    if hasattr(model.config, "use_cache"):
        model.config.use_cache = False

    inputs = arch_wrapper.tokenize_inputs(
        texts=prompts,
        device=device,
        add_special_tokens=add_special_tokens,
    )

    input_ids = as_tensor(inputs["input_ids"], device)
    if input_ids.ndim == 1:
        input_ids = input_ids.unsqueeze(0)

    rows = []

    for b, text in enumerate(prompts):
        tokens = input_ids[b:b+1]
        ids = tokens[0]
        T = ids.shape[0]

        acts, outputs = arch_wrapper.forward_pass(
            input_ids=tokens,
            collect_attn=False,
        )

        has_bos = tokenizer.bos_token_id is not None and ids[0].item() == tokenizer.bos_token_id
        has_eos = tokenizer.eos_token_id is not None and ids[-1].item() == tokenizer.eos_token_id

        if analyze_special_tokens:
            start, end = 0, T
        else:
            start = 1 if has_bos else 0
            end = T - 1 if has_eos else T

        # ---- HARD GUARD: never allow empty / degenerate view ----
        if end <= start:
            start, end = 0, T

        tokens_view = tokens[:, start:end].cpu()

        print(f"[TOKENS VIEW] {tokens_view}")

        if force_include_input and "embedding" in acts:
            hidden_full = acts["embedding"]
            hidden_view = hidden_full[:, start:end-1]

            rec = {
                "prompt_id": b,
                "prompt_text": text,
                "batch_index": batch_index,
                "layer_index": -1,
                "layer_name": "embedding",
                "tokens": tokens_view,
                "slice": {
                    "start": start,
                    "end": end,
                    "has_bos": has_bos,
                    "has_eos": has_eos,
                    "analyze_special_tokens": analyze_special_tokens,
                },
            }

            for m in norm_modes:
                h_norm = normalize_activations(
                    x=hidden_full.clone(), mode=m, block="embedding", layer_index=-1,
                    model_device=arch_wrapper.model_device, model_dtype=arch_wrapper.model_dtype, final_norm=arch_wrapper.final_norm
                )
                #h_view = h_norm[:, start:end-1]
                rec[f"hidden_{m}"] = (
                    arch_wrapper.save_to_fp32(h_norm)
                    if arch_wrapper.fp32_save
                    else h_norm.detach().cpu()
                )
            rows.append(rec)

        layers = sorted(
            (
                arch_wrapper.layer_registry[n]["idx"],
                n,
                acts[n],
            )
            for n in acts
            if n in arch_wrapper.layer_registry
            and arch_wrapper.layer_registry[n]["type"] == "block"
        )

        last_act = None
        last_idx = None

        for idx, name, act in layers:
            hidden_full = act
            hidden_view = hidden_full[:, start:end]

            rec = {
                "prompt_id": b,
                "prompt_text": text,
                "batch_index": batch_index,
                "layer_index": idx,
                "layer_name": f"layer_{idx}",
                "tokens": tokens_view,
                "slice": {
                    "start": start,
                    "end": end,
                    "has_bos": has_bos,
                    "has_eos": has_eos,
                    "analyze_special_tokens": analyze_special_tokens,
                },
            }

            for m in norm_modes:
                h_norm = normalize_activations(
                    x=hidden_full.clone(), mode=m, block="block", layer_index=idx,
                    model_device=arch_wrapper.model_device, model_dtype=arch_wrapper.model_dtype, final_norm=arch_wrapper.final_norm
                )
               # h_view = h_norm[:, start:end-1]
                rec[f"hidden_{m}"] = (
                    arch_wrapper.save_to_fp32(h_norm)
                    if arch_wrapper.fp32_save
                    else h_norm.detach().cpu()
                )

            rows.append(rec)
            last_act = act
            last_idx = idx

        if force_include_output and last_act is not None:
            hidden_full = last_act
            hidden_view = hidden_full[:, start:end-1]
            out_idx = last_idx + 1

            rec = {
                "prompt_id": b,
                "prompt_text": text,
                "batch_index": batch_index,
                "layer_index": out_idx,
                "layer_name": "output",
                "tokens": tokens_view,
                "slice": {
                    "start": start,
                    "end": end,
                    "has_bos": has_bos,
                    "has_eos": has_eos,
                    "analyze_special_tokens": analyze_special_tokens,
                },
            }

            for m in norm_modes:
                h_norm = normalize_activations(
                    x=hidden_full.clone(), mode=m, block="output", layer_index=out_idx,
                    model_device=arch_wrapper.model_device, model_dtype=arch_wrapper.model_dtype, final_norm=arch_wrapper.final_norm
                )
                #h_view = h_norm[:, start:end-1]
                rec[f"hidden_{m}"] = (
                    arch_wrapper.save_to_fp32(h_norm)
                    if arch_wrapper.fp32_save
                    else h_norm.detach().cpu()
                )

            rows.append(rec)

    if save_path is not None:
        save_path = Path(save_path)
        save_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(
            {
                "rows": rows,
                "metadata": {
                    "model_name": getattr(model, "name_or_path", "unknown"),
                    "arch": getattr(arch_wrapper, "arch", "unknown"),
                    "tokenizer_name": getattr(tokenizer, "name_or_path", "unknown"),
                    "batch_index": batch_index,
                    "dataset": dataset,
                    "norm_modes": list(norm_modes),
                    "quantized": arch_wrapper.is_bnb_quantized,
                },
            },
            save_path,
        )

    return rows, {}



# ============================================================
# Hidden for ADL collector
# ============================================================
@torch.no_grad()
def collect_hidden_for_adl(
    arch_wrapper:"LogitLensWrapper",
    all_prompts:List[str],
    custom_tokenizer:PreTrainedTokenizer = None,
    batch_size:int=10,
    save_prefix:str="adl_batch",
    add_special_tokens:bool=False,
    analyze_special_tokens:bool=False,
    force_include_input:bool=True,
    force_include_output:bool=True,
    device:str|None=None,
    norm_modes:Tuple[str, ...]=("raw", "unit_norm", "eps_norm", "model_norm"),
    dataset:str="dataset",
) -> None:

    num_batches = (len(all_prompts) + batch_size - 1) // batch_size
    print(f"[RUN] Processing {len(all_prompts)} prompts in {num_batches} batches of {batch_size}")

    for batch_idx in tqdm(range(num_batches), desc="Running collect hidden for ADL batches"):
        start = batch_idx * batch_size
        end = min((batch_idx + 1) * batch_size, len(all_prompts))
        batch_prompts = all_prompts[start:end]

        save_path = f"{save_prefix}_batch{batch_idx:03d}.pt"

        print(f"\n[batch {batch_idx+1}/{num_batches}] {len(batch_prompts)} prompts → {save_path}")

        try:
            rows, hidden_dict = _collect_hidden_for_adl(
                arch_wrapper=arch_wrapper,
                prompts=batch_prompts,
                custom_tokenizer=custom_tokenizer,
                batch_index=batch_idx,
                add_special_tokens=add_special_tokens,
                analyze_special_tokens=analyze_special_tokens,
                device=device,
                force_include_input=force_include_input,
                force_include_output=force_include_output,
                save_path=save_path,                 
                norm_modes=norm_modes,
                dataset=dataset,
            )

        except RuntimeError as e:
            print(f"[ERROR] Batch {batch_idx} failed: {e}")
            continue

        del rows, hidden_dict, batch_prompts
        torch.cuda.empty_cache()
        gc.collect()

    print(f"\n[DONE] All batches processed and saved for dataset name: {dataset}.")