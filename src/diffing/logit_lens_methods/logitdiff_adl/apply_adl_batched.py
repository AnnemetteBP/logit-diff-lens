...
from ..wrapper import LogitLensWrapper, lmhead_project
...
def _apply_adl(
    ...
    wrapper_A = arch_wrappers[0]
    wrapper_B = arch_wrappers[1]
    delta_lmhead = wrapper_A if lmhead_A else wrapper_B
    ...
    for lid in sorted(layers_A.keys()):
        ...
        for mode in norm_modes:
            ...
            for pid in prompts:
                ...
                # Δh = finetuned - base, assuming A=base and B=finetuned.
                delta = hB - hA
                delta_norm = torch.norm(delta, dim=-1)  # [B, L]

                # projections
                logits_delta, _ = lmhead_project(x=delta, lm_head=delta_lmhead.lm_head, stable=delta_lmhead.stable, model_device=delta_lmhead.model_device)
                logits_A, _ = lmhead_project(x=hA, lm_head=wrapper_A.lm_head, stable=wrapper_A.stable, model_device=wrapper_A.model_device)
                logits_B, _ = lmhead_project(x=hB, lm_head=wrapper_B.lm_head, stable=wrapper_B.stable, model_device=wrapper_B.model_device)
