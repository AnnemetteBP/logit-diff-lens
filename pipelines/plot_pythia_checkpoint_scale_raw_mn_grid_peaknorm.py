#!/usr/bin/env python3
"""Build a peak-normalized Pythia appendix figure with Raw and ModelNorm full-layer scale profiles."""
...
def _peak_normalize(values: list[float]) -> list[float]:
    peak = max(values) if values else 0.0
    if peak <= 0.0:
        return [0.0 for _ in values]
    return [float(v) / peak for v in values]
...
def main() -> None:
        ...
        for col, (checkpoint_key, checkpoint_label) in enumerate(CHECKPOINTS):
            for metric_offset, (metric_key, metric_label) in enumerate(METRICS):
                ...
                for model_key, model_label in MODELS:
                    ...
                    normalized_values = _peak_normalize(values)
    ...
    fig.suptitle("Pythia checkpoint progression across scale (peak-normalized)", y=0.974, fontsize=11.0, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{Peak-normalized full-layer Pythia checkpoint progression across scale under Raw (R) and ModelNorm (MN).} "
        "The layout matches Figure~\\ref{fig:app-pythia-checkpoint-scale-raw-mn}, but each curve is divided by its own peak value so the comparison emphasizes depth-profile shape and peak location rather than absolute magnitude. "
    ...
    (OUT_DIR / "pythia_checkpoint_scale_raw_mn_grid_peaknorm_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("pdf", "png"):
        fig.savefig(OUT_DIR / f"pythia_checkpoint_scale_raw_mn_grid_peaknorm.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
