#!/usr/bin/env python3
"""Build a mirrored JSD comparison figure for LLaMA quantization."""
...
CASE_ORDER = ["hf1bit", "llama_4bit", "llama_8bit"]
CASE_LABELS = {
    "hf1bit": "1.58-bit QAT",
    "llama_4bit": "4-bit PTQ",
    "llama_8bit": "8-bit PTQ",
}
CASE_COLORS = {"hf1bit": "#b94b5f", "llama_4bit": "#4f97b3", "llama_8bit": "#efb6ad"}
...
def main() -> None:
    ...
    fig.suptitle("LLaMA Jensen-Shannon Divergence (JSD)", y=0.978, fontsize=10.8, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{Mirrored layerwise LLaMA quantization JSD profiles on NQ-500.} "
        "For each case, Raw (R) and ModelNorm (MN) are shown as mirrored peak-normalized JSD bar profiles, so the vertical axis emphasizes how divergence is distributed across depth rather than its absolute magnitude. "
        "Dashed lines mark the depth-partition boundaries, and peak JSD values are reported inside each panel to preserve absolute scale. "
        "Across cases, the 8-bit model remains closest to the base model, while 4-bit and especially 1.58-bit show larger and earlier late-depth divergence, with ModelNorm typically making that broader shift more visible than Raw.}\n"
    )
    (OUT_DIR / "llama_quant_depth_story_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"llama_quant_depth_story.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
