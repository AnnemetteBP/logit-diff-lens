#!/usr/bin/env python3
"""Appendix-ready Qwen accuracy figure by readout with base-vs-FT curves."""
...
def main() -> None:
    ...
    handles = [
        ...
        Line2D([0], [0], color="#4f4f4f", linewidth=2.4, label="FT Top-1"),
        Line2D([0], [0], color="#4f4f4f", linewidth=2.4, linestyle=(0, (5, 3)), label="FT Top-5"),
    ]

    fig.subplots_adjust(top=0.81, bottom=0.13, left=0.12, right=0.995)
    fig.suptitle("Qwen layer-wise next-token accuracy across readouts", y=0.972, fontsize=10.1, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{Qwen layer-wise next-token accuracy across readouts on NQ-500.} "
        "Columns show Raw (R) and ModelNorm (MN); rows show the three Qwen fine-tune comparisons. "
        "Within each panel, gray curves show the base model and colored curves show the fine-tuned model, with solid lines for top-$1$ accuracy and dashed lines for top-$5$ accuracy. "
        "This view complements the divergence figures by showing where decoded distribution shifts are also accompanied by weaker predictive agreement with the base model across depth; dashed vertical lines mark the partition boundaries.}\n"
    )
    (OUT_DIR / "qwen_accuracy_by_readout_absolute_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"qwen_accuracy_by_readout_absolute.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
