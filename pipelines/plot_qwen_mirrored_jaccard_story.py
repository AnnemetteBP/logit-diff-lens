#!/usr/bin/env python3
"""Prototype: mirrored J@5 figure for Qwen."""
...
def main() -> None:
    ...
    layers = list(range(len(sample["modes"]["raw"]["jaccard_top5"]["layerwise_mean"])))
    ...
    fig = plt.figure(figsize=(7.15, 2.42), dpi=300)
    grid = GridSpec(1, 3, figure=fig, wspace=0.05)
    axes = [fig.add_subplot(grid[0, i]) for i in range(3)]

    for ax, case in zip(axes, CASE_ORDER):
        raw = payloads[case]["modes"]["raw"]["jaccard_top5"]["layerwise_mean"]
        mn = payloads[case]["modes"]["model_norm"]["jaccard_top5"]["layerwise_mean"]
        raw_peak = max(raw)
        mn_peak = max(mn)
        ...
        ax.set_title(CASE_LABELS[case], fontweight="bold", pad=4)
        ...
        ax.text(0.02, 0.87, "R", transform=ax.transAxes, ha="left", va="center", fontsize=11.0, fontweight="bold")
        ax.text(0.02, 0.09, "MN", transform=ax.transAxes, ha="left", va="center", fontsize=11.0, fontweight="bold")
        ax.text(
            0.98, 0.92, f"R peak {raw_peak:.3f}", transform=ax.transAxes,
            ha="right", va="center", fontsize=10.4, color=color, fontweight="semibold",
            bbox={"facecolor": "white", "edgecolor": "none", "alpha": 0.92, "pad": 0.22},
        )
        ax.text(
            0.98, 0.08, f"MN peak {mn_peak:.3f}", transform=ax.transAxes,
            ha="right", va="center", fontsize=10.4, color="#555555", fontweight="semibold",
            bbox={"facecolor": "white", "edgecolor": "none", "alpha": 0.92, "pad": 0.22},
        )

    axes[0].set_ylabel("Peak-normalized J@5", fontweight="bold", labelpad=4)
    for ax in axes[1:]:
    ...
    fig.subplots_adjust(top=0.72, bottom=0.18, left=0.075, right=0.995)
    fig.suptitle("Qwen Top-5 Jaccard Overlap (J@5)", y=0.975, fontsize=10.3, fontweight="bold")
    fig.supxlabel("Layer", y=0.035, fontsize=9.8, fontweight="bold")
    fig.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, 0.945), ncol=3, frameon=False, handlelength=2.9, columnspacing=1.0)

    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"qwen_mirrored_jaccard_story.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
