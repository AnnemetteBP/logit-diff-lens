#!/usr/bin/env python3
"""Paper-ready LLaMA LogitDiff figure organized by readout and metric."""
...
def main() -> None:
    ...
    for row, (mode_key, mode_title) in enumerate(READOUT_ROWS):
        for col, (metric_key, metric_title) in enumerate(METRIC_COLS):
            ...
            ax.set_ylim(0.0, 0.65 if metric_key == "accuracy" else 1.05)
    ...
    fig.suptitle("LLaMA LogitDiff across readouts and metrics", y=0.942, fontsize=9.4, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{LLaMA LogitDiff across readouts and metrics on NQ-500.} "
        "Rows show Raw (R) and ModelNorm (MN). The first two columns report peak-normalized top-$5$ Jaccard overlap (J@$5$) and Jensen--Shannon divergence (JSD), while the third column shows absolute top-$1$ (solid) and top-$5$ (dashed) next-token accuracy. Colored lines indicate the three low-precision LLaMA variants; in the accuracy column, lighter background traces show the corresponding base-model accuracies under the same readout. Peak normalization in the J@$5$ and JSD columns emphasizes how each signal is distributed across depth rather than its absolute scale, while the accuracy column shows where predictive agreement with the base model remains closer or degrades more strongly across layers. Dashed vertical lines mark the partition boundaries. Across all three variants, the ordering is broadly preserved across readouts, but the apparent localization and relative separation differ between Raw and ModelNorm. JSD separates the variants more strongly than J@$5$, while the readout contrast makes clear that the same quantization family does not induce one readout-invariant depth profile.}\n"
    )
    (OUT_DIR / "llama_dualmetric_by_readout_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"llama_dualmetric_by_readout.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
