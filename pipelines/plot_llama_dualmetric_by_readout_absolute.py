#!/usr/bin/env python3
"""Appendix-ready LLaMA LogitDiff figure organized by readout and metric without peak normalization."""
...
def main() -> None:
    ...
    fig.suptitle("LLaMA LogitDiff across readouts and metrics", y=0.972, fontsize=10.2, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{LLaMA LogitDiff across readouts and metrics on NQ-500 without peak normalization.} "
        "Columns show Raw (R) and ModelNorm (MN); rows show top-$5$ Jaccard overlap (J@$5$) and Jensen--Shannon divergence (JSD). Colored lines indicate the three low-precision LLaMA variants. "
        "Unlike the main-paper companion, the curves retain their absolute scale, making magnitude differences across variants and readouts directly visible; dashed lines mark the partition boundaries. "
        "This appendix view complements the normalized main figure by showing that the 8-bit/4-bit/1.58-bit ordering is visible not only in profile shape but also in absolute decoded divergence and overlap.}\n"
    )
    (OUT_DIR / "llama_dualmetric_by_readout_absolute_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"llama_dualmetric_by_readout_absolute.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
