#!/usr/bin/env python3
"""Build a compact one-column Qwen ModelNorm figure with JSD and J@5."""
...
def main() -> None:
    ...
    fig.suptitle("Qwen ModelNorm (MN)", y=0.947, fontsize=10.8, fontweight="bold")
    ...
    caption = (
        "\\caption{\\textbf{Qwen ModelNorm (MN) LogitDiff on NQ-500.} "
        "Rows show Jensen--Shannon divergence (JSD) and top-$5$ Jaccard overlap (J@$5$) across full depth for the three Qwen fine-tunes. "
        "Within each row, each case is peak-normalized so the figure emphasizes where the decoded signal is concentrated across depth rather than its absolute scale; dashed lines mark the partition boundaries. "
        "All three fine-tunes share a broadly late-depth pattern under ModelNorm, but JSD separates the cases more clearly than J@$5$, indicating that some Qwen differences are more visible in probability mass than in the top-ranked candidate set.}\n"
    )
    (OUT_DIR / "qwen_mn_dualmetric_onecol_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("pdf", "png"):
        fig.savefig(OUT_DIR / f"qwen_mn_dualmetric_onecol.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
