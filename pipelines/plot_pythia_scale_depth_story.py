#!/usr/bin/env python3
"""Build a compact Pythia scale-depth story figure for the paper."""
...
def _draw_split_heatmap(
    ...
    *,
    ...
    ax.tick_params(axis="x", length=0, pad=1, labelsize=8.8)
    ax.tick_params(axis="y", length=0, pad=3, labelsize=8.8)
...
def main() -> None:
    ...
    fig = plt.figure(figsize=(8.5, 6.95), dpi=300)
    outer = GridSpec(2, 1, figure=fig, height_ratios=[2.14, 1.02], hspace=0.075)
    top = outer[0].subgridspec(2, 3, width_ratios=[1.0, 1.0, 0.03], wspace=0.06, hspace=0.22)
    bottom = outer[1].subgridspec(2, 2, height_ratios=[0.13, 1.0], wspace=0.20, hspace=0.0)
    legend_ax = fig.add_subplot(bottom[0, :])
    legend_ax.axis("off")
    fig.subplots_adjust(left=0.08, right=0.975, top=0.90, bottom=0.07)
    ...
    ax_left = fig.add_subplot(bottom[1, 0])
    ax_right = fig.add_subplot(bottom[1, 1])

    fig.text(0.5, 0.968, "Pythia Scale-Depth Patterns", ha="center", va="center", fontsize=13.4, fontweight="bold")
    fig.text(0.5, 0.944, "Each heatmap cell splits 1k on the left and 71k on the right", ha="center", va="center", fontsize=10.8)
    ...
    for metric_key in ("faith", "jsd"):
        ...
        for mode in ("raw", "model_norm"):
            ...
            right_rows = []
            for model_key, _, _ in MODELS:
                left = data[model_key]["1k_first"][f"{mode}:{metric_key}"]
                right = data[model_key]["71k_mid"][f"{mode}:{metric_key}"]
            ...
            _draw_split_heatmap(
                ...
                ylabel="Parameters" if mode == "raw" else None,
                xlabel=(metric_key == "jsd"),
            )
    ...
    ax_left.set_xlabel("Parameters", fontweight="bold")

    ax_right.set_ylabel("Peak JSD depth", fontweight="bold")
    ax_right.yaxis.labelpad = 1
    ax_right.set_xlabel("Parameters", fontweight="bold")
    ...
    legend_ax.legend(handles=legend_handles, loc="center", ncol=4, frameon=False, columnspacing=0.92, handletextpad=0.42, fontsize=9.7)

    caption = (
        "\\caption{Pythia scale--depth patterns on NQ-500 using true block aggregates. "
        "Split-cell heatmaps compare 1k checkpoints (left half of each cell) against 71k checkpoints (right half) across layer partitions and parameter scale. "
        "Bottom summaries compress the same pattern into late-stage faithfulness and the relative depth position at which decoded divergence peaks.}\n"
    )
    (OUT_DIR / "pythia_scale_depth_story_caption.tex").write_text(caption, encoding="utf-8")

    for ext in ("pdf", "png"):
        fig.savefig(OUT_DIR / f"pythia_scale_depth_story.{ext}", dpi=300, bbox_inches="tight", pad_inches=0.04)
    plt.close(fig)
